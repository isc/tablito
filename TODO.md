...
- **Épellation sur voix d'enfant réelle** — le mode vocal épelé de la conjugaison (specs §15.10) apparie en espace phonémique, donc toute transcription homophone est inoffensive par construction. Reste à observer ce que le recognizer produit vraiment sur des noms de lettres isolés dits par un enfant : les ratés (aucune lettre reconnue) sont sans conséquence Leitner mais coûtent une re-demande, et deux d'affilée renvoient au clavier. Si un nom de lettre revient systématiquement mal transcrit, l'ajouter à `names` dans `src/lib/letterNames.ts` suffit (`?voicedebug=1` affiche les transcripts). Priorité moyenne.
  - Pointeur : `src/lib/letterNames.ts`, `src/lib/parseSpelledLetters.ts`
