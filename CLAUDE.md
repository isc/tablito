...
**Clés propres à une langue** : rien n'impose la parité des clés — `LANGS` donne à chaque langue son `build()`. La **conjugaison** en profite : la matière est fr-only (masquée en anglais), donc ses ~214 clés `conj-*` ne sont ajoutées qu'à `buildEntriesFr()`. Elles ne sont pas recopiées dans le script : `loadConjEntries()` transforme `src/lib/conjugationFacts.ts` avec esbuild et appelle `allConjCarrierSentences()` — les phrases porteuses sont des données rédigées à la main, et une copie divergerait en silence. Deux jeux de clés : `conj-<fait>-<rang>` est l'**énoncé** de la question (« Demain, nous… chanter » — l'infinitif, jamais la forme, sinon le MP3 dicte la réponse), `conj-<fait>-0-p` la **phrase complète** lue à l'étape 1 de l'introduction (porteuse 0 seulement, la seule qu'une intro utilise). `scripts/generate-tts.test.mjs` verrouille la couverture (un MP3 manquant ne produit qu'un 404 avalé par `useTTS`, donc une matière muette sans le moindre message d'erreur).
